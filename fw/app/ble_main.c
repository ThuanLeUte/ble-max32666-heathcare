...
static void m_ble_close(ble_msg_t *p_msg)
{
  ...
  BasMeasBattStop((dmConnId_t) p_msg->hdr.param);
}
 ...
 *
 * @param[in]     handler_id  WSF handler ID.handler_id  WSF handler ID.
 *
 * @attention     None
 *
 * @return        None
 */
void FitHandlerInit(wsfHandlerId_t handler_id)
{
  ...
  HrpsInit(handler_id, (hrpsCfg_t *) &fitHrpsCfg);

  // Initialize battery service server
  BasInit(handler_id, (basCfg_t *) &fitBasCfg);
}
 ...
 * @return        None
 */
void FitHandler(wsfEventMask_t event, wsfMsgHdr_t *p_msg)
{
  if (p_msg != NULL)
  {
